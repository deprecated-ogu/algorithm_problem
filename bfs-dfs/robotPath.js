...
const dfs = (M, N, current, count) => {
	...
	dfs(M, N, [row + 1, col], count + 1); // 상
	dfs(M, N, [row - 1, col], count + 1); // 하
	dfs(M, N, [row, col - 1], count + 1); // 좌
	dfs(M, N, [row, col + 1], count + 1); // 우
};

const robotPath = function (room, src, dst) {
  // 다시 방문하지 않기 위해 1로 시작
  dfs(room.length, room[0].length, src, 1);
