...
function getCombination(arr, n, r, depth, tmp, result) {
  if (tmp.length == r) {
    result.push(tmp);
    return
	}
  ...
  for (let i = depth; i < n; i++)
    getCombination(arr, n, r, i + 1, [...tmp, arr[i]], result)
  
  return result;
}

function boringBlackjack(cards) {
	let combination = getCombination(cards, cards.length, 3, 0, [], []);
	let count = 0;
	combination.forEach(element => {
		let sum = 0;
		for (let i = 0; i < element.length; i++) sum += element[i];
