...
const createMinTree = function (arr, ts, te, tree, idx) {
	if (ts === te) {
		tree[idx] = arr[ts];
		return arr[ts];
	}
...
const findMin = (ts, te, rs, re, idx, tree) => {
	if (rs <= ts && te <= re) {
		return tree[idx];
	}
	...
	return Math.min(
		findMin(ts, mid, rs, re, 2 * idx + 1, tree),
		findMin(mid + 1, te, rs, re, 2 * idx + 2, tree)
	);
};
...
const rangeMinimum = function (arr, ranges) {
  ...
  return ranges.map((range) => {
    ...
    return findMin(0, arr.length - 1, start, end, 0, tree);
  });
};
