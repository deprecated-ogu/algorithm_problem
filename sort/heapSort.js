...
function removeRoot(heap) {
	...
	let returnValue = heap.pop();
	while (heap[idx] > heap[leftIdx] || heap[idx] > heap[rightIdx]) {
		if (heap[rightIdx] !== undefined && heap[leftIdx] > heap[rightIdx]) {
...
var output = heapSort([5, 4, 3, 2, 1]);
console.log(output); // --> [1, 2, 3, 4, 5]

var output = heapSort([3, 1, 21]);
console.log(output); // --> [1, 3, 21]

var output = heapSort([4, 10, 3, 5, 1]);
